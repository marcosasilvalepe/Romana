...
(async () => {
	try {
		...
		socket = io(window.location.protocol + "//" + window.location.hostname + ':3100', { autoConnect: false });
		socket.connect();
	
		socket.on('connect', () => socket.emit('test', 'IT WORKED!!'));
		...
		socket.on('transmitting serial data', data => {

			// SET WEIGHT ONLY IF WINDOW IS OPENED
			if (!!document.querySelector('#create-weight__take-weight__weight') === false) return;
			if (parseInt(data.weight) === NaN) return;
				
			document.querySelector('#create-weight__take-weight__weight p').innerText = data.weight;
		});
		...
		// CLOSES THE TAKE WEIGHT MODAL AFTER ACCEPTING THE WEIGHT
		socket.on('new weight updated', async response => {
			try {

				animating = true;

				/*********** TAKE WEIGHT MODAL GETS CLOSED WHEN SERIAL PORT IS CLOSED SO IT DOESN'T NEED TO GET CLOSED HERE ********/

				console.log(response);
				if (response.error !== undefined) throw 'Error al intentar guardar peso bruto. Valor no puede ser 0.'

				// WEIGHT ID DOESN'T MATCH SO NOTHING IS DONE
				const weight_id = parseInt(response.weight_id);
				if (
					!weight_object || 
					weight_object.frozen.id !== weight_id ||
					!!document.querySelector('#create-weight-step-2') === false
				) return;
				...
				const weight_btn = document.getElementById('take-weight-container');
				const cancel_save_btns = document.getElementById('save-cancel-btns');
		...
		socket.on('serial port has been closed', async () => {

			let count = 0;
			while (animating && count < 50) {
				count++;
				await delay(100);
			}

			// CLOSE TAKE WEIGHT MODAL IF IT'S OPEN
			const take_weight_modal = document.querySelector('#create-weight__take-weight_close-modal');
			if (!!take_weight_modal === false) return;
		...
		socket.on('weight created by another user', weight => {
			...
			tr.innerHTML = `
				...
				<td class="created">${sanitize(formatMySQLDate(weight.created))}</td>
				<td class="cycle"></td>
		...
		socket.on('weight status changed by other user', async data => {
			...
			const entities_body = document.querySelector('#companies__entities-list-container .body tbody');
			if (!!entities_body && entities_body.children.length > 0) {
		...
		socket.on('electronic dispatch document - update progress', async data => {
			...
			if (data.screenshot !== null) {
				...
				const div = document.getElementById("electronic-document-screenshots");
				div.style.backgroundImage = `url(${url})`;
			}

			//REMOVE CLASS ENABLED FROM BUTTON IF PROGRESS IS 80 AND DOCUMENT HAS ALREADY BEEN EMITTED
			if (data.progress === 80 && !!document.querySelector('#generate-electronic-document button.enabled.red')) 
				document.querySelector('#generate-electronic-document button.enabled.red').classList.remove('enabled');
			...
			while (progress_container.classList.contains('updating-progress') && data.progress < 100) await delay(10);
			...
			for (let i = current_value + 1; i <= data.progress; i++) {
				...
				if (input.value > data.progress) break;
