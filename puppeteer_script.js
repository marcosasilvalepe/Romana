...
const get_document_rows = doc_id => {
    return new Promise((resolve, reject) => {
        conn.query(`
            ...
            body.price, body.kilos, body.container_code, containers.name AS container_name, body.container_amount, 
            containers.weight AS container_weight
            ...
            for (let i = 0; i < results.length; i++) {
                rows.push({
                    product: {
                        ...
                        name: (results[i].product_code === null) ? '' : results[i].product_name.split('-')[0].toUpperCase().trim(),
                        type: (results[i].product_code === null) ? '' : results[i].product_type.toUpperCase(),
                    ...
                    container: {
                        name: (results[i].container_code === null) ? '' : results[i].container_name.toUpperCase().replace(' CON', '').replace(' PLASTICOS', '').replace(' DE MADERA', ' MADERA'),
...
const get_doc_data = doc_id => {
    return new Promise((resolve, reject) => {
        conn.query(`
            ...
            internal_entities.id1, internal_entities.id2,
...
const go_to_sii = socket => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            socket.emit('loading sii website');
            ...
            socket.emit('electronic document - SII Login successful');

            console.log(global.document.internal_entity.rut);

            //SELECT ENTITY FROM DROPDOWN
            if (global.document.internal_entity.rut === '77.114.753-4' || global.document.internal_entity.rut === '78.447.760-6') {
            ...
            socket.emit('electronic document - destination entity done');
            ...
            socket.emit('electronic document - document date done');
            ...
            socket.emit('electronic document - destination city done');
            ...
            socket.emit('electronic document - vehicle data done');
            ...
            socket.emit('electronic document - header done');

            //ADD DOCUMENT BODY DETAILS
            let i = 0;
            
            const progress = Math.ceil(30 / rows.length);
            ...
            for (let row of rows) {
                ...
                if (row.product.name === null) {
                    ...
                    await page.keyboard.type(row.container.name.split(' ')[0]);
                ...
                //CONTAINER WITH PRODUCT
                    ...
                    await page.focus(`#myTable input[name="EFXP_NMB_0${i + 1}"]`);
                    await page.keyboard.type(row.product.name);
                    ...
                    await page.waitForSelector(`#rowDescripcion_0${i + 1} textarea`);
                    await page.focus(`#rowDescripcion_0${i + 1} textarea`);
                    await page.keyboard.type(row.product.cut + ' ' + row.container.amount + ' ' + row.container.name);
                    ...
                    await page.keyboard.type(row.product.kilos.toString()); 
                ...
                socket.emit('electronic document - row done', progress);

                i++;
            }

            socket.emit('electronic document - body done');
            ...
            await page.focus(`input[name="EFXP_QTY_${last_row_number}"]`);
            await page.keyboard.type('1');

            await page.focus(`input[name="EFXP_UNMD_${last_row_number}"]`);
            await page.keyboard.type('UN');

            await page.focus(`input[name="EFXP_PRC_${last_row_number}"]`);
            await page.keyboard.type('1');
            await page.keyboard.press('Tab');
            ...
            socket.emit('electronic document - document done');
            ...
            socket.emit('electronic document - document signed');
            ...
            else fs.writeFileSync('./pdf_test2.pdf', data.pdf, 'binary');

            socket.emit(`electronic document - file saved`, );
...
module.exports = { generate_document } 